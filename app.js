  ...
  const rows = state.entries.length === 0
    ...
    : state.entries.map(entry => {
      ...
      const sigHtml = entry.signature_name
        ? `<div class="sig-block">
             <span class="sig-stamp">SIGNED</span><br>
             ${entry.signature_img ? `<img src="${entry.signature_img}" style="max-height:35px; margin:4px 0; border-bottom:1px solid #ccc;"><br>` : ''}
      ...
      const piHtml = entry.pi_approval_name
        ? `<div class="sig-block">
             ...
             ${entry.pi_approval_img ? `<img src="${entry.pi_approval_img}" style="max-height:35px; margin:4px 0; border-bottom:1px solid #ccc;"><br>` : ''}
      ...
      return `<tr>
        ...
        <td data-label="Actions" class="no-print">
          <div class="row-actions">
            ${canStaffSign ? `<button class="btn ghost small" onclick="openSignaturePad('${entry.id}','staff')">Sign Pad</button>` : ''}
            ${canPiApprove ? `<button class="btn ghost small" onclick="openSignaturePad('${entry.id}','pi')">PI Approve Pad</button>` : ''}
...
function openSignaturePad(entryId, kind) {
  closeModal();
  const isPi = kind === 'pi';
  const div = document.createElement('div'); div.id = 'modalRoot';
  document.body.appendChild(div);
  ...
  <div class="overlay">
    <div class="modal" style="max-width:450px;">
      <h3>${isPi ? 'PI Approval Signature' : 'Staff E-Signature'}</h3>
      <div class="modal-sub">Draw your signature below using mouse or touchscreen.</div>
      <div class="field">
        <label>Full Name Confirmation</label>
        <input id="sig-typed-name" value="${escapeHtml(currentUser.name)}">
      </div>
      <div class="field">
        <label>Signature Pad</label>
        <div style="border: 2px dashed #004D40; border-radius: 6px; background:#fff; touch-action: none;">
          <canvas id="sig-canvas" width="380" height="150" style="width:100%; height:150px; cursor:crosshair;"></canvas>
        </div>
      </div>
      <div style="display:flex; justify-content:space-between; align-items:center; margin-top:8px;">
        <button class="btn ghost small" id="sig-clear">Clear Pad</button>
        <span style="font-size:11px; color:#666;">Recorded: ${new Date().toLocaleTimeString()}</span>
      </div>
      <div class="modal-actions" style="margin-top:16px;">
        <button class="btn secondary" id="sig-cancel">Cancel</button>
        <button class="btn" id="sig-save">Submit Signature</button>
      </div>
    </div>
  </div>`;
  ...
  document.getElementById('sig-cancel').addEventListener('click', closeModal);

  document.getElementById('sig-save').addEventListener('click', async () => {
    const typedName = document.getElementById('sig-typed-name').value.trim();
    if (!typedName) { alert('Please confirm your name.'); return; }
    const imgData = canvas.toDataURL("image/png");

    const updateData = isPi ? {
      pi_approval_name: typedName,
      pi_approval_timestamp: nowIso(),
      pi_approval_img: imgData
    } : {
      signature_name: typedName,
      signature_timestamp: nowIso(),
      signature_img: imgData
    };

    const { error } = await supabaseClient.from('delegation_entries').update(updateData).eq('id', entryId);

    if (error) {
      alert("Database error: " + error.message);
    } else {
      await loadStudyEntries(activeStudyId);
      await logAudit(activeStudyId, isPi ? 'PI approval signed' : 'Staff signature recorded', `Signed by ${typedName}`);
      closeModal();
      render();
    }
  });
