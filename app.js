    ...
    : state.entries.map(entry => {
      ...
      const sigHtml = entry.signature_name
        ? `<div class="sig-block">
             ...
             ${entry.signature_img ? `<img src="${entry.signature_img}" style="max-height:40px; margin:4px 0; border-bottom:1px dashed #004D40;"><br>` : ''}
      ...
      const piHtml = entry.pi_approval_name
        ? `<div class="sig-block">
             ...
             ${entry.pi_approval_img ? `<img src="${entry.pi_approval_img}" style="max-height:40px; margin:4px 0; border-bottom:1px dashed #004D40;"><br>` : ''}
      ...
      return `<tr>
        ...
        <td data-label="Actions" class="no-print">
          <div class="row-actions">
            ${canStaffSign ? `<button class="btn ghost small" onclick="openCanvasModal('${entry.id}','staff')">✍️ Sign Canvas</button>` : ''}
            ${canPiApprove ? `<button class="btn ghost small" onclick="openCanvasModal('${entry.id}','pi')">✍️ PI Approve Canvas</button>` : ''}
...
/* ---------------- Canvas Drawing Modal (Signature Pad) ---------------- */
function openCanvasModal(entryId, kind) {
  ...
  <div class="overlay">
    <div class="modal" style="max-width:440px;">
      <h3>${isPi ? 'PI Approval Signature' : 'Staff Digital Signature'}</h3>
      <div class="modal-sub">Draw your signature inside the box using mouse or touch.</div>
      
      <div class="field" style="margin-bottom:12px;">
        <label>Full Name</label>
        <input id="cnv-name" value="${escapeHtml(currentUser.name)}">
      </div>
      
      <div class="field">
        <label>Signature Box (Canvas)</label>
        <div style="border: 2px solid #004D40; border-radius: 6px; background:#fff; position:relative;">
          <canvas id="paintCanvas" width="380" height="160" style="width:100%; height:160px; cursor:crosshair; touch-action:none;"></canvas>
        </div>
      </div>
      
      <div style="display:flex; justify-content:space-between; align-items:center; margin-top:6px;">
        <button class="btn ghost small" id="cnv-clear">Clear Canvas</button>
        <span style="font-size:11px; color:#888;">GCP Compliant Audit</span>
      </div>

      <div class="modal-actions" style="margin-top:16px;">
        <button class="btn secondary" id="cnv-cancel">Cancel</button>
        <button class="btn" id="cnv-save">Save & Sign</button>
      </div>
    </div>
  </div>`;
